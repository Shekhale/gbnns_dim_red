            ...
            for lam in lambdas:
                for r_pos in ranks_pos:
                    for r_neg in ranks_neg:
                        if r_pos < r_neg + 1:
                            learning_params.append((margin, lam, dint, r_pos, r_neg))

    for margin, lam, dint, r_pos, r_neg in learning_params:
        ...
        if args.print_results > 0:
            with open(results_file_name, "a") as rfile:
                ...
                rfile.write(
                    ...
                    (args.database, xt.shape[0], args.batch_size, args.dout, val_k, lam, r_pos, r_neg, dint, args.optim, margin))
                # rfile.write("\n")
                log = all_logs[-1]
                rfile.write(
        ...
        if args.save == "save":
            torch.save(net.state_dict(),
                       "/mnt/data/shekhale/data/" + args.database + "/" + args.database + "_net_" + params_string + ".pth")
