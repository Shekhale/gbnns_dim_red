...
from dim_red.support_func import  loss_permutation, loss_top_1_in_lat_top_k, normalize_numpy,\
                          ...
                          repeat, pairwise_NNs_inner,get_nearestneighbors_partly, save_transform
...
def train_triplet(xb, xt, xv, xq, args, results_file_name):
    ...
    if args.database == "sift":
        # lambdas = [10]
        lambdas = [0.0003162] + list(np.logspace(-3, -2, 5)) + list(np.logspace(-2, 1, 5))[1:]
        ranks_pos = [5]
        ranks_neg = [10]
        lrs = [0]
        # lrs = [0.001, 0.0001]
        # ranks_pos = [2, 5, 20, 50, 100]
        bss = [1024]
        # bss = [1024, 256, 64]
    ...
    for lambda_uniform, dint, r_pos, r_neg, lr, bs in learning_params:
        ...
        if args.save > 0:
            knn_low_path = "/mnt/data/shekhale/models/nns_graphs/" + args.database + "/knn_1k_triplet_wrap.ivecs"
            get_nearestneighbors_partly(yb, yb, 1000, args.device, bs=3*10**5, needs_exact=True, path=knn_low_path)
            save_transform(xb, net, args.database + "/" + args.database + "_base_triplet_wrap.fvecs", args.device)
            save_transform(xq, net, args.database + "/" + args.database + "_query_triplet_wrap.fvecs", args.device)

        if args.database == "sift":
            c_support.get_graphs_and_search_tests("t", "s", dim, args.dout, xq.shape[0], "", 10**6, True)
            with open(results_file_name, "a") as rfile:
                rfile.write("\n")
                rfile.write("------------------------------------------------------ \n")